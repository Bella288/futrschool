...
function setupAssignmentEventListeners() {
  ...
  // Get the filtered and sorted assignments to match what's displayed
  const filteredAssignments = filterAssignments(assignments, currentSearchTerm);
  const displayedAssignments = sortAssignments(filteredAssignments, currentSortMethod);

  // Mark complete
  list.querySelectorAll(".complete-btn").forEach(btn => {
    btn.addEventListener("click", (e) => {
      const displayIndex = parseInt(btn.dataset.index);
      const assignmentToComplete = displayedAssignments[displayIndex];
      
      // Find the actual index in the original array
      const actualIndex = assignments.findIndex(a => 
        a.title === assignmentToComplete.title && 
        a.due === assignmentToComplete.due && 
        a.category === assignmentToComplete.category
      );
  ...
  list.querySelectorAll(".edit-grade-btn").forEach(btn => {
    btn.addEventListener("click", (e) => {
      const displayIndex = parseInt(btn.dataset.index);
      const assignmentToEdit = displayedAssignments[displayIndex];
      
      // Find the actual index in the original array
      const actualIndex = assignments.findIndex(a => 
        a.title === assignmentToEdit.title && 
        a.due === assignmentToEdit.due && 
        a.category === assignmentToEdit.category
      );
  ...
  list.querySelectorAll(".delete-btn").forEach(btn => {
    btn.addEventListener("click", (e) => {
      const displayIndex = parseInt(btn.dataset.index);
      const assignmentToDelete = displayedAssignments[displayIndex];
      ...
      if (confirm(`Are you sure you want to delete "${assignmentToDelete.title}"?`)) {
